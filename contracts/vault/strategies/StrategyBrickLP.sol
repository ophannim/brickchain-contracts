 ...
 *
 * This strategy simply deposits whatever funds it receives from the vault into the selected Builder pool.
 * BRICK rewards from providing liquidity are farmed every few minutes, sold and split 50/50.
 * The corresponding pair of assets are bought and more liquidity is added to the Builder pool.
 */
contract StrategyBrickLP is Ownable, Pausable {
     ...
     * {treasury} - Address of the Brick treasury
    ...
    address public constant treasury =
        address(0x4A32De8c248533C28904b24B4cFCFE18E9F2ad01);
     ...
     *
     * {REWARDS_FEE} - 3% goes to BRICK holders through the {rewards} pool.
     ...
     * {brickToWbnbRoute} - Route we take to get from {brick} into {wbnb}.
     * {wbnbToBrickRoute} - Route we take to get from {wbnb} into {brick}.
     * {brickToLp0Route} - Route we take to get from {brick} into {lpToken0}.
     * {brickToLp1Route} - Route we take to get from {brick} into {lpToken1}.
     */
    address[] public brickToWbnbRoute = [brick, wbnb];
    address[] public wbnbToBrickRoute = [wbnb, brick];
    address[] public brickToLp0Route;
    address[] public brickToLp1Route;
    ...
    constructor(
        ...
        if (lpToken0 == wbnb) {
            brickToLp0Route = [brick, wbnb];
        } else if (lpToken0 != brick) {
            brickToLp0Route = [brick, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            brickToLp1Route = [brick, wbnb];
        } else if (lpToken1 != brick) {
            brickToLp1Route = [brick, wbnb, lpToken1];
        }
     ...
     * 3. It swaps the {brick} token for {lpToken0} & {lpToken1}
    ...
    function chargeFees() internal {
        uint256 toWbnb =
            IERC20(brick).balanceOf(address(this)).mul(45).div(1000);
    ...
    /**
     * @dev Swaps {brick} for {lpToken0}, {lpToken1} & {wbnb} using PancakeSwap.
     */
    function addLiquidity() internal {
        uint256 cakeHalf = IERC20(brick).balanceOf(address(this)).div(2);
